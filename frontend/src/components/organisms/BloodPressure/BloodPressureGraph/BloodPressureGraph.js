...
export const BloodPressureChart = ({ data, month }) => {
  const sysDataAll = data?.map((measurement) => {
  ...
  const diaDataAll = data?.map((measurement) => {
  ...
  const pulseDataAll = data?.map((measurement) => {
    return { x: measurement.date, y: Number(measurement.diastolic) }
  })
            ...
            <VictoryAxis
              ...
              tickFormat={(x) => {
                return (
                  ...
                  '.' +
                  x.toString().slice(5, 7)
                )
              }}
              style={{ grid: { stroke: '#BEBEBE' }, tickLabels: { angle: 90 } }}
            />
            <VictoryLine
              style={{
                data: { stroke: '#c43a31' },
